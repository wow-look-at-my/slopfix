...
[rules](rules/README.md) holds the prose tables as XML, split by purpose. Nothing reads them at run time: `cmd/rulegen` parses the folder during the build and writes Go, compiling each pattern with [go-regex-compiler](https://github.com/wow-look-at-my/go-regex-compiler). [table](table/table.go) is what that generated Go is made of.
