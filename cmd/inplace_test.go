...
func edit(path, old, replacement string) map[string]any {
...
func TestAnEditInsideAFenceIsJudgedWhereItLands(t *testing.T) {
	...
	got := ask(t, edit(path, "slopfix fmt docs/*.md     # join", "slopfix purge .           # move"))
	if got.out != nil {
...
func TestAnEditToProseIsRepairedWhereItLands(t *testing.T) {
	path := onDisk(t, "doc.md", "# Use\n\nThe gate is open.\n")
	got := ask(t, edit(path, "The gate is open.", "The gate is shut; the write fails."))
