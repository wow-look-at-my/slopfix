...
func TestStopIsRefusedAfterFourCloselySpacedIdenticalTurns(t *testing.T) {
	...
	// The refusal names the ways out, because a single that only says "do not"
	// costs a round trip while the reader guesses. It names no scheduling tool:
	// an ordinary web session has none, and sending the reader after a single
	// it cannot call is the same wasted round trip in a different direction.
