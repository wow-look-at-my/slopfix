...
// rules passes them here, so nothing has to swap a package-level value and put
// it back. go-toolchain runs a package's quick tests in parallel, and a test
// that mutates a global loses to whichever sibling restores it.
...
// command itself does. It travels as an argument so no path here reads a
// package-level rule set, which a test cannot replace safely.
