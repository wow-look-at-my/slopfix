...
func hasFileRead(f *syntax.File) bool {
	return hasStatementCall(f, func(c *syntax.CallExpr) bool {
		...
		if e.name != "cat" && e.name != "head" && e.name != "tail" && e.name != "sed" {
			return false
		}
		for _, w := range c.Args[e.index+1:] {
			s, st := literal(w)
			if st && !strings.HasPrefix(s, "-") && !strings.HasPrefix(s, "/") {
				return true
			}
			if st && strings.HasPrefix(s, "/") && !strings.HasPrefix(s, "/proc/") && !strings.HasPrefix(s, "/sys/") && !strings.HasPrefix(s, "/dev/") {
				return true
			}
		}
		return false
...
func hasBadRM(f *syntax.File) bool {
	return hasStatementCall(f, func(c *syntax.CallExpr) bool {
		...
		for _, w := range c.Args[e.index+1:] {
			s, st := literal(w)
			if st && strings.HasPrefix(s, "-") && s != "--" && !regexp.MustCompile(`^--(recursive|force|verbose|interactive)$|^-[rRfvIi]+$`).MatchString(s) {
				return true
			}
		}
		return false
...
func stripStages(s *syntax.Stmt, names map[string]bool) {
	b, ok := s.Cmd.(*syntax.BinaryCmd)
	if !ok {
		return
	}
	if b.Op == syntax.Pipe {
		if c, ok := b.Y.Cmd.(*syntax.CallExpr); ok && len(c.Args) > 0 && names[cmd(c)] {
			s.Cmd = b.X.Cmd
		}
	}
}
func stripOrTrue(s *syntax.Stmt) {
	b, ok := s.Cmd.(*syntax.BinaryCmd)
	if ok && b.Op == syntax.OrStmt && cmdCall(b.Y.Cmd) == "true" {
		s.Cmd = b.X.Cmd
	}
}
